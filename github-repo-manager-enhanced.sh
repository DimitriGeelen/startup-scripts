...
# 5. Clean a local repo and pull a fresh copy
# 6. Update the local repo with changes
# 7. Upload local changes to the remote repo
#
# Features:
...
#
# Usage:
# 1. Make this script executable: chmod +x github-repo-manager-enhanced.sh
# 2. Run the script: ./github-repo-manager-enhanced.sh
...
# Clean and pull function - NEW FUNCTION
clean_and_pull_repo() {
    echo -e "${BLUE}Cleaning and pulling fresh copy of repository: ${YELLOW}$local_repo_name${NC}"
    ...
    if ! git diff --quiet || ! git diff --staged --quiet; then
        echo -e "${YELLOW}You have uncommitted changes in this repository.${NC}"
        echo -e "${RED}WARNING: This operation will discard all local changes. Continue? (y/n)${NC}"
    ...
    echo -e "${BLUE}Cleaning local repository...${NC}"
    git clean -fd
    
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to clean local repository.${NC}"
        cd ..
        return 1
    fi
    
    # Reset all changes, including staged ones
    echo -e "${BLUE}Resetting all local changes...${NC}"
    ...
    echo -e "${GREEN}Repository cleaned and updated successfully!${NC}"
...
# Update local repo with changes function - NEW FUNCTION
update_repo_with_changes() {
    echo -e "${BLUE}Updating local repository with remote changes: ${YELLOW}$local_repo_name${NC}"
    ...
    if ! git diff --quiet || ! git diff --staged --quiet; then
        ...
        echo -e "1) Stash changes before updating"
        echo -e "2) Try to merge changes"
        ...
        case $update_choice in
            1)
                ...
                if [ $? -ne 0 ]; then
                    echo -e "${RED}Failed to apply stashed changes. There might be conflicts.${NC}"
            ...
            2)
                # Try direct merge
                echo -e "${BLUE}Attempting to merge remote changes with your local changes...${NC}"
                git pull origin "$current_branch"
                
                if [ $? -ne 0 ]; then
                    echo -e "${RED}Failed to merge. You'll need to resolve conflicts manually.${NC}"
                    cd ..
                    return 1
                fi
                ;;
...
# Upload local changes to remote repo function - NEW FUNCTION
upload_local_changes() {
    echo -e "${BLUE}Uploading local changes to remote repository: ${YELLOW}$local_repo_name${NC}"
    ...
    if ! git diff --quiet || ! git diff --staged --quiet; then
        ...
        if [[ "$commit_choice" =~ ^[Yy]$ ]]; then
            ...
            echo -e "${YELLOW}Cannot push without committing changes. Operation cancelled.${NC}"
            cd ..
            return 1
        fi
    fi
    ...
    if [ -n "$remote_rev" ] && [ "$local_rev" != "$remote_rev" ]; then
        echo -e "${YELLOW}Your local branch is not in sync with the remote branch.${NC}"
        echo -e "${BLUE}Do you want to: ${NC}"
        echo -e "1) Pull remote changes first (recommended)"
        echo -e "2) Force push your changes (may overwrite remote work)"
        echo -e "3) Cancel operation"
        read -r sync_choice
        ...
        case $sync_choice in
            1)
                # Pull changes first
                echo -e "${BLUE}Pulling remote changes first...${NC}"
            ...
            2)
                # Force push warning
                echo -e "${RED}WARNING: Force pushing will overwrite remote history!${NC}"
                echo -e "${RED}This is generally not recommended unless you're sure. Continue? (y/n)${NC}"
    ...
    echo -e "${GREEN}Changes uploaded to remote successfully!${NC}"
...
show_main_menu() {
    ...
    echo -e "5) Clean a local repo and pull a fresh copy"
    echo -e "6) Update the local repo with changes"
    echo -e "7) Upload local changes to the remote repo"
    echo -e "0) Exit"
    echo -e "${BLUE}====================================${NC}"
    ...
    case $menu_choice in
        ...
        4)
            ...
            ;;
        5)
            # Clean and pull fresh copy mode
            if find_local_repos && select_local_repository; then
                clean_and_pull_repo
            fi
            ;;
        6)
            # Update with changes mode
            if find_local_repos && select_local_repository; then
                update_repo_with_changes
            fi
            ;;
        7)
            # Upload local changes mode
            if find_local_repos && select_local_repository; then
                upload_local_changes
            fi
            ;;
